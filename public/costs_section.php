...
if ($section === 'cek') {
    ...
    $cek = Reports::costMarginCheck($ym, platformFilter(), $warnPct, $badPct);
    $ringkas = ['wajar' => 0, 'periksa' => 0, 'parah' => 0, 'rugi' => 0];
    foreach ($cek as $c) {
        $ringkas[$c['status']]++;
    }
    ?>
    <div class="kpis" style="margin-bottom:14px">
      <div class="kpi ok">
        ...
        <div class="hint">marjin sampai <?= number_format($warnPct, 0, ',', '.') ?>%</div>
      </div>
      <div class="kpi <?= $ringkas['periksa'] > 0 ? 'bad' : '' ?>">
        <div class="label">Perlu dicek</div>
        <div class="value"><?= num($ringkas['periksa']) ?></div>
        <div class="hint">marjin di atas <?= number_format($warnPct, 0, ',', '.') ?>%</div>
      </div>
    ...
    <form method="get" class="filters" style="margin-bottom:12px" action="costs.php">
      ...
      <div class="field">
        <label>Batas wajar (%)</label>
        <input type="number" name="warn" value="<?= e((string) $warnPct) ?>" min="0" max="100" step="1">
      </div>
      <div class="field">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr>
          ...
          <th class="num">Marjin</th><th>Catatan</th>
        </tr></thead>
        <tbody>
        <?php
        $badge = ['wajar' => 'ok', 'periksa' => 'warn', 'parah' => 'bad', 'rugi' => 'bad'];
        $teks  = ['wajar' => 'Wajar', 'periksa' => 'Perlu dicek', 'parah' => 'Sangat tidak wajar', 'rugi' => 'Jual rugi'];
        ...
        <?php endforeach; ?>
        <?php if ($cek === []): ?>
          <tr><td colspan="9" class="muted">Belum ada produk yang HPP-nya terisi pada rentang ini.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
